...
class AgeBinaryConfig(BaseModel):
    ...
    version: str = "v1.3.1"
    checksums_url: HttpUrl = "https://github.com/FiloSottile/age/releases/download/v1.3.1/sha256sums.txt"
...
class GitConfig(BaseModel):
    ...
    dotfiles_repo: Optional[str] = None
...
def create_default_config() -> BootstrapConfig:
    ...
    if system == "darwin":
        profile = "work"
        console.print(f"[cyan]Detected macOS - using 'work' profile[/cyan]")
    else:
    ...
    console.print("This will be saved to ~/.config/sealbridge/bootstrap.yaml for future runs.")
...
def load_config(path: Optional[Path] = None, allow_missing: bool = False) -> Optional[BootstrapConfig]:
    """
