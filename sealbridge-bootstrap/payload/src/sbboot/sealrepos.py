...
def install_sealrepos(config: BootstrapConfig, policy_manager) -> None:
    ...
    sealreposctl_path = paths.get_bin_dir() / ("sealreposctl.exe" if paths.is_windows() else "sealreposctl")
    if sealreposctl_path.exists():
        console.print("✅ [green]SealBridge Repos appears to already be installed.[/green]")
        return
    ...
    if repo_dir.exists() and (repo_dir / ".git").exists():
        ...
        repo_url = None
        if hasattr(config, "sealrepos") and isinstance(getattr(config, "sealrepos", None), dict):
            repo_url = config.sealrepos.get("repo_url") or None
        if not repo_url:
            repo_url = os.environ.get("SEALBRIDGE_REPOS_URL")
        if not repo_url:
            raise SealreposError("SealBridge Repos URL not provided. Set SEALBRIDGE_REPOS_URL or configure in bootstrap config.")
    ...
    console.print("✅ [bold green]SealBridge Repos installed successfully.[/bold green]")
...
def create_config_interactive(
    ...
    if work_hosts_input:
        ...
        repos_config["network"]["vpn_required"] = repos_config["network"]["reachability"]["work_hosts"]
    ...
    if Confirm.ask("Do you want to configure repositories now?", default=False):
        ...
        while True:
            ...
            local_path = Prompt.ask(
                "Local path",
                default=f"${{HOME}}/workspace/code/{repo_name}"
            )
            ...
            home_url = Prompt.ask("Home remote URL (e.g., git@github.com:user/repo.git)")
            
            work_url = Prompt.ask("Work remote URL (e.g., ssh://git@ghe.example.com/org/repo.git)", default="")
            ...
            if work_url:
                ...
                recipients_input = Prompt.ask(
                    "GPG fingerprints for work remote (comma-separated)",
                    default=""
                )
                if recipients_input:
