...
def get_age_binary(config: "BootstrapConfig") -> Path:
    ...
    if version in ["v1.1.1", "1.1.1", "v1.2.0", "1.2.0"]:
        ...
        console.print(f"[yellow]Please update your config file at:[/yellow] {paths.get_default_config_path()}")
    ...
    try:
        ...
        try:
            ...
            checksum_response = httpx.get(str(config.age.binary.checksums_url), timeout=10.0)
            ...
            if expected_checksum:
                ...
                console.log(f"[yellow]Warning:[/yellow] Checksum not found for '{asset_name}', skipping verification")
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 404:
                console.log(f"[yellow]Warning:[/yellow] Checksums file not available, skipping verification")
            else:
                raise
        except Exception as e:
            console.log(f"[yellow]Warning:[/yellow] Could not fetch checksums: {e}, skipping verification")
