...
from sbboot.config import AgeConfig, AgeBinaryConfig
from sbboot.errors import AgeBinaryError

@pytest.fixture
def age_config() -> AgeConfig:
    return AgeConfig(
        binary=AgeBinaryConfig(
            version="v1.2.0",
            checksums_url="https://example.com/age/v1.2.0/sha256sums.txt"
        ),
        encrypted_key_path="assets/id_bootstrap.age"
    )

def test_get_age_binary_found(mocker: MockerFixture, age_config: AgeConfig):
    ...
    assert agewrap.get_age_binary(age_config) == Path("/fake/bin/age")

def test_get_age_binary_download_and_verify(mocker: MockerFixture, age_config: AgeConfig, tmp_path: Path):
    ...
    binary_path = agewrap.get_age_binary(age_config)

    assert binary_path == tmp_path / "age"

def test_get_age_binary_checksum_mismatch(mocker: MockerFixture, age_config: AgeConfig, tmp_path: Path):
    mocker.patch("sbboot.paths.get_bin_dir", return_value=tmp_path)
    mocker.patch("sbboot.agewrap._get_system_arch", return_value="linux-amd64")
    mocker.patch("sbboot.util.download_file")
    ...
    with pytest.raises(AgeBinaryError):
        agewrap.get_age_binary(age_config)
