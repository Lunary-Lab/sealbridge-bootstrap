...
from sbboot.errors import OtpError
...
def test_verify_totp_failure_bad_code(otp_config: OtpGateConfig, mocker: MockerFixture):
    ...
    mock_response = httpx.Response(
        200, json={"ok": False, "error": "Invalid token"}, request=mocker.Mock()
    )
    mocker.patch("httpx.Client.post", return_value=mock_response)
...
def test_verify_totp_http_client_error(
    otp_config: OtpGateConfig, mocker: MockerFixture
):
    mocker.patch.dict(os.environ, {"TEST_SECRET_ENV_VAR": "supersecret"})

    mock_response = httpx.Response(
        400, json={"error": "Bad request"}, request=mocker.Mock()
    )
    mocker.patch("httpx.Client.post", return_value=mock_response)
...
def test_verify_totp_http_server_error_with_retry(
    otp_config: OtpGateConfig, mocker: MockerFixture
):
    mocker.patch.dict(os.environ, {"TEST_SECRET_ENV_VAR": "supersecret"})
...
def test_verify_totp_exhausts_retries(otp_config: OtpGateConfig, mocker: MockerFixture):
    ...
    mocker.patch(
        "httpx.Client.post", side_effect=httpx.ConnectError("Connection failed")
    )
    mocker.patch("time.sleep")
...
def test_get_client_secret_missing_env_var(otp_config: OtpGateConfig):
    ...
    # Should return None if not set and prompt_if_missing is False
    assert otp._get_client_secret(otp_config) is None
